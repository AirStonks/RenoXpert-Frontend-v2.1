...
- **If layout types exist:** replace the flat package grid with a two-level UI:
  ...
  2. **On selected layout:** a **renderings gallery** (H6 — the layout's `rendering_images`, e.g. a responsive grid / lightbox reusing the Sub-project-C modal pattern) and the layout's **sub-package cards** — the existing package-card UI (radio, `booking_amount` "Booking Fee", "View Quotation"), plus **"Start from RM{getInitialDownPayment(pkg.order)}"** (H3). `selectedPackage` + `handlePackageChange` + booking flow stay intact (scoped to the selected layout's packages).
