...
- **Lint is strict:** `npm run lint` runs `--max-warnings 0`. `react-refresh/only-export-components` (warn, `allowConstantExport: true`) means a module that exports a React component must not also export a **function** — keep `buttonClasses` (a function) in its own non-component file, and do not create a barrel that mixes component + function exports.
- **No automated test suite exists** (scripts are only `dev`/`build`/`lint`/`preview`). Per task, the verification cycle is: `npm run lint` (clean) → `npm run build` (`tsc -b && vite build` succeeds) → visual parity vs the mockup board at widths 375 / 768 / 1280 → a behavior-preservation review (handlers/IDs/logic unchanged). Do not scaffold a test runner (out of scope, YAGNI).
