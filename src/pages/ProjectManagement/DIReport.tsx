...
function DIReport() {
    ...
    const handleBackClick = () => {
        if (state) {
            ...
            navigate(LOCAL_PATH_PREFIX + 'di-forms');
        }
    };
    ...
    useEffect(() => {
        ...
        const getDIForm = async () => {
            setIsLoading(true);
            try {
                const response = await fetchDIForm(formId);
                const data: DefectInspectionForm = response?.data;

                if (response?.success) {
                    setDiForm(data);

                    if (data.reno_progress_id) {
                        getRenoProgress(Number(data.reno_progress_id));
                    }
                }
            } catch (error) {
        ...
        const getRenoProgress = async (renoProgressId: number) => {
            try {
                ...
                notify('error', 'Failed to fetch Reno Progress!');
            }
                                ...
                                !isLoading ?
                                    diForm?.area ?
                                        ...
                                        <div className=" flex justify-center items-center">
                                            <span>DI Form Detail Not Available</span>
                                        </div>
                                    :
                                    ''
                            }
