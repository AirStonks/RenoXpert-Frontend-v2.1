...
function RPMDetailV2({ renoProgress, setRenoProgress, permissions, users, setUsers }: Props) {
    ...
    const handleChangeStatus = async (e: React.ChangeEvent<HTMLSelectElement>, id: number) => {
        ...
        try {
            const response = await changeTaskStatus(Number(renoProgress.id), id, status);
    ...
    const handleToggleVisibility = async (taskId: number) => {
        setIsLoading(true);
        try {
            const response = await toggleTaskVisibility(Number(renoProgress.id), taskId);
            if (response?.success) {
    ...
    const removeServerExternalFile = async (taskId: number, documentIndex: number) => {
        ...
        try {
            const response = await removeExternalTaskDocument(Number(renoProgress.id), taskId, documentIndex);
    ...
    const uploadExternalFiles = async (taskId: number) => {
        ...
        try {
            const response = await uploadTaskExternalDocuments(Number(renoProgress.id), taskId, pendingExternalUploadItems);
