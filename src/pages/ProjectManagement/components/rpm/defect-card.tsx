...
export const DefectCard = ({ setSelectedTask, defectJob, handleStatusChange }: Props) => {
    ...
    return (
        <div className="shadow-md rounded-xl overflow-hidden bg-white w-full">
            <div className="bg-gradient-to-r from-amber-50 to-amber-100 border-b border-gray-200 p-2 px-4">
                <div className="flex items-center justify-between">
                    <span className="text-base font-semibold">Defect</span>
                    <span className="text-xs text-gray-500">{completedTasks}/{totalTasks}</span>
                </div>
                <div className="w-full bg-gray-200 rounded-full h-1 mt-1">
                    <div className="bg-amber-600 h-1 rounded-full" style={{ width: `${completionPercentage}%` }}></div>
                </div>
            </div>
            <div className="p-2">
