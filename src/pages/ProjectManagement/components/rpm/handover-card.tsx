...
export const HandoverStatusCard = ({ setSelectedTask, handoverJob, handleStatusChange }: Props) => {
    const completedTasks = handoverJob.rpm_tasks?.filter((task) => task.status === "completed").length || 0;
    const totalTasks = handoverJob.rpm_tasks?.length || 0;
    ...
    return (
        <div className="shadow-md rounded-xl overflow-hidden bg-white w-full">
            <div className="bg-gradient-to-r from-slate-50 to-slate-100 border-b border-gray-200 p-2 px-4">
                <div className="flex items-center justify-between">
                    <span className="text-base font-semibold">Handover Status</span>
            ...
            <div className="p-2">
                <div className="h-[200px] overflow-y-auto">
                    <ul className="space-y-1">
                        {handoverJob.rpm_tasks?.map((task) => (
                            <li
                                key={task.id}
