...
import { toggleDIFormReportLinkStatus } from '../../../../services/api';
...
function DIRLinkManagementModal({ diForm, setDiForm }: Props) {
    ...
    useEffect(() => {
        if (diForm) {
            const target = document.getElementById('clipboard_2_target') as HTMLInputElement;
            const button = document.getElementById('clipboard_2_button') as HTMLButtonElement;
    ...
    const handleChangeLinkStatus = async () => {
        ...
        try {
            const response = await toggleDIFormReportLinkStatus(Number(diForm?.id));

            if (response?.success) {
                notify('success', 'Link status changed successfully!');
    ...
    return (
        <>
            ...
            <div className="modal p-14" data-modal="true" id="dir_link_mgnt_modal">
...
export default DIRLinkManagementModal