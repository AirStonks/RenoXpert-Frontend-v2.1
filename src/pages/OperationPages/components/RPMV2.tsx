...
import { changeTaskStatus, fetchTaskDocuments, liveUploadTaskAttachment, removeTaskDocument, updateSelectedTaskComments, uploadTaskDocuments } from '../../../services/operationApi';
...
function RPMV2({ renoProgress, setRenoProgress }: RPMV2Props) {
    ...
    const removeServerFile = async (taskId: number, documentIndex: number) => {
        ...
        try {
            const response = await removeTaskDocument(Number(renoProgress.id), taskId, documentIndex);
    ...
    const uploadFiles = async (taskId: number) => {
        ...
        try {
            const response = await uploadTaskDocuments(Number(renoProgress.id), taskId, pendingUploadItems);
    ...
    const handleOpenDocumentModal = async (taskId: number, phase: string, job: string, task: string) => {
        ...
        try {
            const response = await fetchTaskDocuments(Number(renoProgress.id), taskId);
    ...
    const handleChangeTaskComment = async () => {
        ...
        try {
            const response = await updateSelectedTaskComments(Number(renoProgress.id), Number(selectedTask.id), selectedTask.owner_comment, selectedTask.internal_comment);
    ...
    const handleChangeTaskStatus = async (e: React.ChangeEvent<HTMLSelectElement>, selectedTaskId: number) => {
        ...
        try {
            const response = await changeTaskStatus(Number(renoProgress.id), Number(selectedTaskId), status);
            const updatedTask: JobTask = response.data;
